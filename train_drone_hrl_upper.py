...
from env.DroneNavigationEnv_hrl_meta import DroneNavigationEnv
...
def train_hrl():
    ...
    config = {
        ...
        "low_level_state_dim": 37,  # 4 + 5 * 6 + 3
        "action_con_dim": 2,
        ...
        "meta_state_dim": 37,  # 高低层使用相同的状态表示
    ...
    for episode in range(max_train_episodes):
        ...
        while not (terminated or truncated):
            ...
            high_level_mask = np.ones(env.NUM_HIGH_LEVEL_TASKS, dtype=np.int8)
    ...
    if episode > 0 :
        meta_agent.save(f"./models_hrl/meta_agent.pth")
