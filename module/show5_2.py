...
class GDOPCalculator:
    ...
    def calculate_gdop_for_fixed_target(self,
                                        fixed_target_point_3d: np.ndarray,  # Expects 3D: [x, y, z]
                                        current_sensor_network_3d: List[np.ndarray],
                                        # Expects list of 3D: [[x,y,z], ...]
        ...
        Args:
            fixed_target_point_3d: 计算GDOP的固定目标点坐标 [x, y, z]
            current_sensor_network_3d: 当前使用的传感器位置列表 (包括已存在的和假设新增的), each [x,y,z]
            sensor_weights_for_fixed_target: current_sensor_network_3d中各传感器相对于fixed_target_point_3d的权重

        Returns:
            加权GDOP值 (PDOP in this 3D spatial context)
        """
        if not isinstance(fixed_target_point_3d, np.ndarray) or fixed_target_point_3d.shape != (3,):
            # Ensure target is a 3D numpy array
            if isinstance(fixed_target_point_3d, (list, tuple)) and len(fixed_target_point_3d) == 3:
            ...
            else:  # Default to a 2D with z=0 if not properly formatted, or handle error
                print("Warning: fixed_target_point_3d not properly formatted for 3D. Assuming z=0 if 2D.")
                ...
                else:  # Fallback if completely malformed
        ...
        if not current_sensor_network_3d or len(current_sensor_network_3d) < 3 or \
            ...
            # Need at least 3 sensors for 3D GDOP (actually 4 for full determination if clock is unknown, but 3 for 3x3 GtWG)
            return float('inf')
        ...
        for i, sensor_loc_3d in enumerate(current_sensor_network_3d):
            if not isinstance(sensor_loc_3d, np.ndarray) or sensor_loc_3d.shape != (3,):
                # Ensure sensor_loc_3d is a 3D numpy array
                if isinstance(sensor_loc_3d, (list, tuple)) and len(sensor_loc_3d) == 3:
                    sensor_loc_3d = np.array(sensor_loc_3d)
                else:  # Default to a 2D with z=0 if not properly formatted
                    print(f"Warning: sensor_loc_3d {i} not properly formatted for 3D. Assuming z=0 if 2D.")
            ...
            G_list.append(unit_vector)  # unit_vector is [ux, uy, uz]
            valid_weights_for_G.append(sensor_weights_for_fixed_target[i])

        if len(G_list) < 3:  # Need at least 3 non-coplanar sensors for a 3x3 GtWG matrix
            return float('inf')

        G_matrix = np.array(G_list)  # Shape: (num_valid_sensors, 3)
        W_matrix = np.diag(valid_weights_for_G)

        try:
            GtWG = G_matrix.T @ W_matrix @ G_matrix  # Shape: (3, 3)
            if GtWG.shape != (3, 3) or np.linalg.det(GtWG) < 1e-10:
                return float('inf')

            GtWG_inv = np.linalg.inv(GtWG)
            # For 3D spatial coordinates, this calculates PDOP (Position Dilution of Precision)
...
def reward_function_2d(x_coords_mesh, y_coords_mesh, K_val, sigma_val, xc_val, yc_val):
    # This function remains 2D as the reward is a projection on the X-Y plane
...
def calculate_sensor_placement_gdop_3d(
        ...
        z_coord_new_sensor  # Z-coordinate for the potential new sensor
):
...
# Reward Targets (X,Y Coords) and their Sigmas
target1_coords_xy_reward_center = np.array([400.0, 600.0])
sigma_target1 = 5  #sim_area_width / 10.0
target2_coords_xy_reward_center = np.array([600.0, 400.0])
sigma_target2 =  10  #sim_area_width / 15.0
...
for i in range(num_existing_sensors):
    ...
    existing_fixed_sensors_3d.append(np.array([sensor_x, sensor_y, Z_SENSOR_HEIGHT]))  # Add Z_SENSOR_HEIGHT
    print(f"  已存在的传感器 {i + 1}: ({sensor_x:.2f}, {sensor_y:.2f}, {Z_SENSOR_HEIGHT:.1f})")

# Define THE SPECIFIC FIXED TARGETS for GDOP Situation A evaluation (3D)
...
# 4. Calculate SEPARATE "Sensor Placement GDOP Heatmaps" for T1 and T2 (using 3D coordinates)
gdop_heatmap_T1 = calculate_sensor_placement_gdop_3d(
    ...
    Z_SENSOR_HEIGHT  # New sensors are also at Z_SENSOR_HEIGHT
)
...
existing_sensors_array_vis_xy = np.array([s[:2] for s in existing_fixed_sensors_3d])  # Get XY for scatter plot
...
plt.scatter(existing_sensors_array_vis_xy[:, 0], existing_sensors_array_vis_xy[:, 1],
            c='red', s=100, marker='^', label='已存在的固定传感器 (XY投影)', edgecolors='black')
plt.scatter(fixed_target_T1_for_gdop_3d[0], fixed_target_T1_for_gdop_3d[1],
            c='magenta', s=150, marker='X', label=f'GDOP固定目标点 T1 (XY投影)', edgecolors='black')
plt.colorbar(im_gdop_T1, label=f'目标T1的3D PDOP值 (Clipped at {gdop_display_clip_val})')
plt.xlabel('新传感器的潜在X坐标 (m)');
plt.ylabel('新传感器的潜在Y坐标 (m)')
plt.title(f'传感器放置GDOP效益图 (针对目标T1, 3D计算)')
...
plt.scatter(existing_sensors_array_vis_xy[:, 0], existing_sensors_array_vis_xy[:, 1],
            c='red', s=100, marker='^', label='已存在的固定传感器 (XY投影)', edgecolors='black')
plt.scatter(fixed_target_T2_for_gdop_3d[0], fixed_target_T2_for_gdop_3d[1],
            c='cyan', s=150, marker='P', label=f'GDOP固定目标点 T2 (XY投影)', edgecolors='black')
plt.colorbar(im_gdop_T2, label=f'目标T2的3D PDOP值 (Clipped at {gdop_display_clip_val})')
plt.xlabel('新传感器的潜在X坐标 (m)');
plt.ylabel('新传感器的潜在Y坐标 (m)')
plt.title(f'传感器放置GDOP效益图 (针对目标T2, 3D计算)')
plt.legend(fontsize='small');
plt.grid(True, linestyle='--', alpha=0.5);
plt.show()
...
def plot_local_3d_comparison(target_name, target_xy_coords_val, original_reward_component,
    ...
    fig_local.suptitle(
        f"目标点 {target_name} ({target_xy_coords_val[0]},{target_xy_coords_val[1]}) 附近局部3D奖励对比 (针对其自身的3D PDOP效益遮罩)",
        fontsize=12)
...
plot_local_3d_comparison("T1 (奖励中心)", target1_coords_xy_reward_center, reward_T1_original, final_reward_T1_masked,
...
plot_local_3d_comparison("T2 (奖励中心)", target2_coords_xy_reward_center, reward_T2_original, final_reward_T2_masked,
