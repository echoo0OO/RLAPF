...
class DroneNavigationEnv(gym.Env):
    ...
    def _calculate_reward(self,transmitted_data_in_bits=0.0):
        """
               【最终优化版 - 简洁高效】
        ...
        # --- 2. 行为导向的正向奖励 ---
        # c. 成功通信奖励：直接、即时地奖励数据传输行为。
        #    权重设为较高，以明确这是主要目标之一。
        data_transmission_reward = 1.2 * (transmitted_data_in_bits / 1e6)  # 每传输1Mbit，奖励 1.0
        # d. 成功定位奖励 (Progress Reward)：奖励任何能有效降低不确定性的行为。
        #    这是“精细化定位”的主要驱动力。
        localization_progress_reward = 0.0
        if self.radii_before_action is not None:
            # ADD: Calculate current estimated radii (all sensors)
            radii_after = self.uncertainty_model.uncertainty_radii
            # ADD: Calculate total reduction in estimated radii for all sensors
            radii_reduction = np.sum(self.radii_before_action - radii_after)

            # ADD: Reward positive reduction in radii (adjust weight as needed)
            localization_progress_reward = 0.8 * max(0, radii_reduction)

        # --- 3. 整合总奖励 ---
        total_reward = (time_step_penalty +
                        delay_penalty +
                        data_transmission_reward +
                        localization_progress_reward)
    ...
    def _is_in_comm_range(self):
        ...
        for idx in np.where(eligible_mask)[0]:
            ...
            radii = 100 # Cap radius for stability
    ...
    def _execute_communication(self):
        ...
        if np.all(np.isinf(dist_to_consider)): return
    ...
    def step(self, high_level_action):
        ...
        for i in range(self.max_task_commitment_steps):
            ...
            self.errors_before_action = np.linalg.norm(
                self.uncertainty_model.estimated_positions - self.sensor_true_positions,
                axis=1
            )
