...
class ActorCritic_Hybrid(nn.Module):
    def __init__(self,
        ...
        self.actor_con = nn.Sequential(
            ...
            nn.Linear(mid_dim[1], mid_dim[2]),
            nn.Tanh(),
            nn.Linear(mid_dim[2], action_con_dim),
        )
        ...
        self.actor_dis = nn.Sequential(
            ...
            nn.Linear(mid_dim[1], mid_dim[2]),
            nn.Tanh(),
            nn.Linear(mid_dim[2], action_dis_dim * action_dis_len),
            nn.Softmax(dim=-1)
        )
    ...
    def get_logprob_entropy(self, state, action_dis, action_con):  
        ...
        # Assuming act_dis is [batch_size, action_dim]
        # logp_dis should match [batch_size, action_dim]
...
class PPO_Abstract:
    def __init__(self,
        ...
        self.buffer = PPOBuffer(state_dim, action_dis_dim, action_dis_len, action_con_dim, buffer_size, gamma, lam, device)

        self.agent = ActorCritic_Discrete(state_dim, action_dis_dim, mid_dim).to(device)
        self.agent_old = ActorCritic_Discrete(state_dim, action_dis_dim, mid_dim).to(device)
...
class PPO_Hybrid(PPO_Abstract, ABC):
    def __init__(self, state_dim, action_dis_dim, action_dis_len, action_con_dim, mid_dim, lr_actor, lr_critic, lr_decay_rate, buffer_size, target_kl_dis, target_kl_con,
                 gamma, lam, epochs_update, v_iters, eps_clip, max_norm, coeff_entropy, random_seed, device,
                 lr_std, init_log_std, if_use_active_selection):
        super().__init__(state_dim,  action_dis_dim, action_dis_len, action_con_dim, mid_dim, lr_actor, lr_critic, lr_decay_rate, buffer_size, target_kl_dis, target_kl_con,
                         gamma, lam, epochs_update, v_iters, eps_clip, max_norm, coeff_entropy, random_seed, device)

        self.agent = ActorCritic_Hybrid(state_dim, action_dis_dim, action_dis_len, action_con_dim, mid_dim, init_log_std).to(device)
        self.agent.apply(weight_init)
        self.agent_old = ActorCritic_Hybrid(state_dim, action_dis_dim, action_dis_len, action_con_dim, mid_dim, init_log_std).to(device)
        self.agent_old.load_state_dict(self.agent.state_dict())
    ...
    def compute_loss_pi(self, data):
        ...
        logp_con = logp_con.gather(1, act_dis).squeeze()
        
        # Now, flatten the batch and action dimensions for ratio calculations
        ...
        print(f"agent.log_std: {self.agent.log_std}")
